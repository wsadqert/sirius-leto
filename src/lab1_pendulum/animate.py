...
def _animation_step(_frame: int, alpha_array: Sequence[float | np.float64], config: dict[str, ...]) -> tuple[mpl.artist.Artist, ...]:
	"""
	...
	"""
	global time_previous_frame, pendulum_line, pendulum_point, time_text, n, frame, fps  # noqa - variables are defined in `animate()`
...
def animate(config: dict[str, ...]) -> None:  # is_in_demo: bool = False
	...
	:param config: Dictionary, containing information about a laboratory (see more in `datastore/lab1_pendulum/README_input.md`).
	"""
	global n, pendulum_line, pendulum_point, time_text, alpha_array  # noqa - variables will be defined below

	# extracting values from config
	globals().update(config)

	logging.info("Reading data from file")
	logging.debug(f"Read from {datapath_model}")

	# reading data generated by `model.py`
	globals().update(_read_dataset(datapath_model))

	logging.info("Setting up tkinter")

	# setting up tkinter
	if plot_alpha and plot_animation:
		root = tk.Tk()
	...
	# show angle plot
	if plot_alpha:
		logging.debug("Setting up subplot with plot")
