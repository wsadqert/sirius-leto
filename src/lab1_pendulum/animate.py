...
import src  # noqa do not delete! see line 61
from src.general.constants import *
from src.general.calculations import *


def __animation_step(frame: int, constants_module, alpha_array) -> tuple:
	global t0

	# obtaining required variables from `constants_module`
	pendulum_axis_x = constants_module.pendulum_axis_x
	pendulum_axis_y = constants_module.pendulum_axis_y
	frames_count = constants_module.frames_count
	render_dt = constants_module.render_dt
	dt = constants_module.dt
	l = constants_module.l
...
def animate(mode: Literal["basic", "windage"], plot_animation: bool = True, plot_alpha: bool = True) -> None:
	global t0, pendulum_line, pendulum_point, time_text, n  # noqa
	...
	if plot_animation:
		...
		animation = FuncAnimation(fig,  # noqa, no comments :)
		                          func=__animation_step,
		                          fargs=(constants_local, alpha_array),
		                          interval=0,
		                          frames=n,
		                          blit=True,
		                          repeat=False,
